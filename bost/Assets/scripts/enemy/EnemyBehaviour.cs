...
public class EnemyBehaviour : MonoBehaviour
{
    ...
    public void damagedgroundfix()
    {
       
        {
            if (isGround == false) m_Animator.CrossFadeInFixedTime(Plusstatename + "damaged2", 0f);
    ...
    public bool groundcheck()
    {
        ...
        void UpdateTimers()
        {
            if (m_TimeSinceLastTargetView > 0.0f) //毎フレーム呼び出しで、視認判から規定時間立ってなかったら減算
        ...
        public bool CheckForObstacle(float forwardDistance)
        {
            ...
            if (Physics2D.CircleCast(m_Collider.bounds.center, m_Collider.bounds.extents.y - 0.7f, m_SpriteForward, forwardDistance, m_Filter.layerMask.value)) //特定レイヤーのものが地震の方向距離内にあればtrue
            {
            Debug.Log("dost");
                return true;
            }
        ...
        if (facing == -1)
        {
           ...
           // this.transform.localScale = new Vector3( m_SpriteForward.x, 1, 1);
            ...
            skeletonAnimation.skeleton.ScaleX = spriteFaceLeft ? 1 : -1;
            m_SpriteForward = spriteFaceLeft ? Vector2.left : Vector2.right;
            this.transform.localScale = new Vector3(m_SpriteForward.x*0.4f, 0.4f, 1);
        }
        }
    ...
    public void UpdateFacing()
        ...
        {                                           //タイマーをリセットする。タイマーが0になったら追跡終了
            ...
            if (toTarget.sqrMagnitude < viewDistance * viewDistance) //視認距離内にいる場合
            {
                Vector3 testForward = Quaternion.Euler(0, 0, spriteFaceLeft ? -viewDirection : viewDirection) * m_SpriteForward; //既定の視認方向が左の場合、□方法を変える
                if (skeletonAnimation.skeleton.FlipX) testForward.x = -testForward.x;

                float angle = Vector3.Angle(testForward, toTarget); //水平方向からプレイヤーの角度

                if (angle <= viewFov * 0.6f)
                {
                    //we reset the timer if the target is at viewing distance. 角度内なら視認中を維持
                    m_TimeSinceLastTargetView = timeBeforeTargetLost;
                }
            }
         Debug.Log(toTarget);
        ...
        Debug.Log("attack");
        if ((Player.position - transform.position).sqrMagnitude < (meleeRange * meleeRange) && Mathf.Sign(Player.position.x - transform.position.x)==m_SpriteForward.x)
        {
    ...
    public void Shooting()
        ...
        damaged = true;
        Debug.Log(isGround);
        Debug.Log(damager.downstate);
        
            if (isGround == false) {m_Animator.CrossFadeInFixedTime(Plusstatename + "damaged2", 0f);
       

        }
        else  m_Animator.CrossFadeInFixedTime(Plusstatename+"damaged",0f);
        setSKelAnimation("damaged");
        Vector2 throwVector = new Vector2(5f, 2.0f);
         Vector2 damagerToThis = damager.transform.position - transform.position;
         this.transform.DOShakePosition(0.2f,0.3f,15,1,false,true);
            throwVector.x = Mathf.Sign(damagerToThis.x) * -4.0f;
