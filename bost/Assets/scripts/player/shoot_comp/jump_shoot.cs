...
public class jump_shoot : Shoot_Component
{
    ...
    public override void air_updateframes(Rigidbody2D rb2d, float newHorizontalMovement, Animator animator, Transform parent,  SkeletonAnimation skeletonAnimation)
    {
        ...
        if (clipInfo[0].clip.name == "idle" || clipInfo[0].clip.name == "run" || clipInfo[0].clip.name == "jumpdown" || clipInfo[0].clip.name == "jumpup")
        {
            if (Playerinput.Instance.Intract.Up)
            {
                ...
                if (pools == null)
                    pools = parent.gameObject.GetComponent<bullet1_pool>();
                DOVirtual.DelayedCall(0.0f, () => rb2d.AddForce(new Vector2(0f, 500f)), false);
                animator.CrossFadeInFixedTime("boltshoot2", 0f);
