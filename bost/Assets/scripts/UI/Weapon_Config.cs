...
public class Weapon_Config : MonoBehaviour
{
    ...
    public GameObject Slector_Main_LR;
    public GameObject currentMainselect_LR;
    ...
    public void OnClickUp()
    {
        ...
        else if (states == States.LR)
        {
            currentMainselect_LR.transform.position = MaiselectText_Left.transform.position;
    ...
    public void OnClickDown()
    {
        ...
        {
            currentMainselect_LR.transform.position = MaiselectText_Right.transform.position;
    ...
    public void OnClickChoice() //トリガースキルの状態のみ管理　残りポイントを見てcurrentkamaに追加する　各スキルのnextattacknameを変更する　SCの更新のみkamapの野良べ替えはattackwrapper
    {
        ...
        if(states == States.Mainset)
        {
            ...
            {
                Slector_Main_LR.SetActive(true);
                states = States.LR;
                currentMainselect_LR.transform.position = MaiselectText_Left.transform.position;
                return;
            }
        ...
        if (states == States.LR)
        {
            if (currentMainselect_LR.transform.position == MaiselectText_Left.transform.position)
            {
                AttackWrapper.Instance.setleftitem_name(currentselect.GetComponent<EquipIcon>().Getskillnamel());
                AttackWrapper.Instance.WeaponSetter(currentselect.GetComponent<EquipIcon>().Getskillnamel());
                states = States.None;
                Slector_Main.SetActive(false);
                Slector_Main_LR.SetActive(false);
                return;
            }

            if (currentMainselect.transform.position == MaiselectText_Right.transform.position)
            {
                AttackWrapper.Instance.setrightitem_name(currentselect.GetComponent<EquipIcon>().Getskillnamel());
                AttackWrapper.Instance.WeaponSetter(currentselect.GetComponent<EquipIcon>().Getskillnamel());
                states = States.None;
                Slector_Main.SetActive(false);
                Slector_Main_LR.SetActive(false);
